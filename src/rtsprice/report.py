...
def write_report_md(
    ...
    if diffs:
        # Пустая строка обязательна: на чистом прогоне — без предупреждений и
        # без причин отсева — заголовок встал бы вплотную к последней строке
        # таблицы, и разметка проглотила бы его как ещё одну ячейку.
