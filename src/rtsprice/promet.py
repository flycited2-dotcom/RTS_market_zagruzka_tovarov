...
def normalize(text: object) -> str:
    ...
    Транслитерация, а не отбрасывание незнакомых букв: «Рубеж» и «РУБЕЖ»
    обязаны сойтись, а при отбрасывании от них остаётся «ре», и сходится
    что угодно. На этом однажды уже был получен неверный вывод, будто
    четверти моделей нет в прайсе.
    """
    return re.sub(r"[^a-z0-9]", "",
                  "".join(_RU.get(c, c) for c in str(text).lower()))
