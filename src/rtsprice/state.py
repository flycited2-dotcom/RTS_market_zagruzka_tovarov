...
def deletion_rows(
    ...
    """Строки для позиций, исчезнувших из выгрузки.

    ``keep_prefixes`` — префиксы всех источников, которые в этом запуске не
    пересобирались: не только замороженных, но и отсеянных ключами --only и
    --skip, сломанных сменой колонок, потерявших файл, убранных из конфига.
    Их позиции отсутствуют в выгрузке не потому, что товар кончился, а потому
    что его не читали, и снимать их нельзя.
    """
