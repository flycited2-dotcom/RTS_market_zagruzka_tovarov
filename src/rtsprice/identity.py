...
class IdMap:
    def __init__(self, path: Path) -> None:
        ...
        #: Идентификаторы, прочитанные из файла. Нужны, чтобы отличить
        #: «карты не было» от «карту урезали»: во втором случае максимум в
        #: префиксе цел, нумерация выглядит здоровой, а часть соответствий
        #: исчезла — и молча получит новые номера.
        self.loaded_ids = set(self._by_key.values())
